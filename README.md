...
Movie Data Analysis
Introduction This notebook performs an exploratory data analysis on a movie dataset to uncover insights related to movie profitability, language performance, genre distribution, key personnel (producers, directors), and actors' impact.

Data Source The analysis is based on the imdb_data.csv dataset, which contains various attributes of movies including budget, revenue, genres, cast, crew, and more.
...
Filtered movies with a budget greater than 0.
Calculated profit as revenue - budget.
Defined helper functions to extract director, producer, and actors from the 'crew' and 'cast' columns, which contain string representations of lists of dictionaries.
Identified the movie with the maximum profit using idxmax().
Key Finding:
...
Calculated ROI as profit / budget.
Grouped the DataFrame by original_language and calculated the mean ROI.
...
Iterated through the 'genres' column, which contains string representations of lists of dictionaries.
Used ast.literal_eval to safely parse these strings into lists.
Extracted the 'name' of each genre and added it to a set to ensure uniqueness.
Key Finding:
...
Extracted director and producer names using the previously defined helper functions.
Grouped the DataFrame by producer and calculated the mean ROI.
Sorted the results in descending order and selected the top 3.
Key Finding:
...
Used explode() on the 'actors' column to get a series of all individual actors.
Counted the occurrences of each actor to find the one with the most movie appearances.
Filtered the DataFrame to show movies featuring this top actor and displayed their titles and profits.
Key Finding:
...
Identified the top 3 directors based on the count of movies they directed.
For each of these directors, filtered their movies.
Exploded the 'actors' column for their movies and counted the appearances of each actor.
Displayed the top actor for each director.
Key Finding:
